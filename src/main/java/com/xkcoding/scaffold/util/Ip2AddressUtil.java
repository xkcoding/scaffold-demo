...
public class Ip2AddressUtil {
	...
	public static String getAddressInLocal(String ip) {
		...
		try {
			...
			// 过滤为 0 的数据
			List<String> regionListFilter = regionList.stream().filter(s -> !StrUtil.equals("0", s)).distinct().collect(Collectors.toList());
			// 再用 | 拼接回来
	...
	public static String getAddressOnNetByTaoBao(String ip) {
		...
		try {
			...
			if (ObjectUtil.equal(code, 0)) {
				...
				// 过滤为 XX 以及为空的数据
				List<String> addressInfoFilter = addressInfo.stream().filter(s -> StrUtil.isNotBlank(s) && !StrUtil.equals("XX", s)).distinct().collect(Collectors.toList());
				// 用 | 拼接回来
