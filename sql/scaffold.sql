...
CREATE TABLE sys_dept (
  ...
  status INT ( 2 ) DEFAULT 1 COMMENT '部门状态（0停用 1正常）',
...
CREATE TABLE sys_menu (
  ...
  menu_type INT ( 2 ) DEFAULT NULL COMMENT '菜单类型（0目录 1菜单 2按钮）',
  visible INT ( 2 ) DEFAULT 1 COMMENT '菜单状态（0隐藏 1显示）',
  path VARCHAR ( 200 ) DEFAULT '' COMMENT 'vue路由',
  component VARCHAR ( 100 ) DEFAULT '' COMMENT 'vue组件',
...
CREATE TABLE sys_dict_data (
  ...
  is_default INT ( 2 ) DEFAULT 0 COMMENT '是否默认（0否 1是）',
  status INT ( 2 ) DEFAULT 1 COMMENT '状态（0停用 1正常）',
  create_by VARCHAR ( 64 ) DEFAULT '' COMMENT '创建者',
  create_time datetime COMMENT '创建时间',
  update_by VARCHAR ( 64 ) DEFAULT '' COMMENT '更新者',
...
CREATE TABLE sys_login_log (
  ...
  status INT ( 2 ) DEFAULT 1 COMMENT '登录状态（0失败 1成功）',
